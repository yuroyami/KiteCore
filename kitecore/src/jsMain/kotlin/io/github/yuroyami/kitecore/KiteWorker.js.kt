 ...
 * ```
 */
public class KiteWorker private constructor(private val worker: dynamic) {
        ...
        worker.onmessage = { event: dynamic ->
            ...
            if (data.id == pendingId) {
                ...
                if (data.ok == true) {
                    ...
                    p?.completeExceptionally(KiteWorkerException("" + data.error))
                }
            }
            Unit
        }
    ...
    public companion object {
        ...
        public fun of(jobJs: String): KiteWorker = KiteWorker(makeBlobWorker(bootstrap(jobJs)))

        // Protocol v2: request { id, payload } -> reply { id, ok, result | error },
        // result normalized with '' + so undefined round-trips as "undefined".
        // Keep in sync with the wasmJs KiteWorker AND kmp-ssot's
